    ...
    loginForm.onsubmit = async function(e) {
        ...
        try {
            ...
            if (res.ok && data.token) {
                ...
                toastr.success('Login successful! Redirecting...', 1000);
                setTimeout(() => {
...
if (document.getElementById('reset-password-form')) {
    ...
    resetPasswordForm.onsubmit = async function(e) {
        ...
        try {
            ...
            const data = await res.json();
            if (res.ok) {
                toastr.success('Password reset successful! Redirecting to login...', 2000);
                setTimeout(() => {
...
function handleVerifyEmailHashRoute() {
    if (window.location.hash.startsWith('#verify-email')) {
        if (typeof toastr !== 'undefined') {
            toastr.info('Verifying your email...', 0);
        }
            ...
            fetch(`${API_BASE_URL}/api/Auth/verify-email?token=${encodeURIComponent(token)}`)
                ...
                .then(({ ok, data }) => {
                    if (ok) {
                        ...
                        setTimeout(() => {
                            window.location.href = '/Frontend/html/auth/account-activated.html';
                        }, 1000);
                        return;
                    } else {
                        if (typeof toastr !== 'undefined') {
