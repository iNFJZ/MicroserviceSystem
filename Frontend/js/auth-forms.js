...
window.addEventListener("DOMContentLoaded", async function() {
  ...
  if (emailElem) {
    ...
    if (token) {
      try {
        const res = await fetch(`${API_BASE_URL}/Auth/validate-reset-token?token=${encodeURIComponent(token)}`);
        const data = await res.json();
...
if (document.getElementById("register-form")) {
    ...
    registerForm.addEventListener("submit", async function(e) {
        ...
        const phoneNumber = sanitizeInput(document.getElementById("register-phoneNumber")?.value);
        ...
        try {
            ...
            if (res.ok) {
                if (data.username && data.username !== username) {
                    showToast(window.i18next.t("usernameAutoGenerated").replace("{original}", username).replace("{generated}", data.username), false);
                } else {
                ...
                if (data.errors && Array.isArray(data.errors)) {
                    ...
                    if (errorMessage.includes("already exists")) {
                        if (errorMessage.includes("Username")) {
                            showToast(window.i18next.t("usernameAlreadyExists").replace("{username}", username), true);
                        } else {
